...
class WriteCheck
{
    ...
    public static function add(
        ...
        $stmt->execute([$nextId, $cv_no, $check_no, $account_id, $payee_type, $ref_no, $payee_id, $write_check_date, $payee_address, $memo, $gross_amount, $discount_amount, $net_amount_due, $vat_percentage_amount, $net_of_vat, $tax_withheld_amount, $total_amount_due]);
    }
    ...
    public static function find($id)
    {
        ...
        // Query to get the main check details
        $stmt = $connection->prepare('
            SELECT 
                wc.id AS check_id,
                wc.account_id,
                ...
                wc.payee_id,
                wc.payee_address,
                wc.memo AS check_memo,
                wc.gross_amount,
                wc.net_amount_due,
                wc.discount_amount AS check_discount_amount,
                wc.vat_percentage_amount,
                wc.net_of_vat,
                wc.tax_withheld_amount,
                wc.total_amount_due,
                coa.account_name,
                CASE 
            ...
            FROM 
                wchecks wc
            LEFT JOIN 
                chart_of_account coa ON wc.account_id = coa.id
            LEFT JOIN 
        ...
        $check = [
            ...
            'cv_no' => $checkData['cv_no'],
            'account_name' => $checkData['account_name'],
            'ref_no' => $checkData['ref_no'],
            'payee_type' => $checkData['payee_type'],
            'write_check_date' => $checkData['write_check_date'],
            'payee_id' => $checkData['payee_id'],
            'payee_address' => $checkData['payee_address'],
            'memo' => $checkData['check_memo'],
            'gross_amount' => $checkData['gross_amount'],
            'discount_amount' => $checkData['check_discount_amount'],
            'net_amount_due' => $checkData['net_amount_due'],
            'vat_percentage_amount' => $checkData['vat_percentage_amount'],
            'net_of_vat' => $checkData['net_of_vat'],
            'tax_withheld_amount' => $checkData['tax_withheld_amount'],
            'total_amount_due' => $checkData['total_amount_due'],
            'payee_name' => $checkData['payee_name'],
            'details' => []
        ];

        // Query to get the transaction details
        $stmt = $connection->prepare('
            SELECT 
                wcd.transaction_id,
                wcd.account_id AS details_account_id,
                ...
                wcd.taxable_amount,
                wcd.input_vat,
                coa.account_name AS account_name_details
            FROM 
                wchecks_details wcd
            LEFT JOIN 
                chart_of_account coa ON wcd.account_id = coa.id
            WHERE 
                wcd.transaction_id = :id
        ');
        ...
        foreach ($details as $detail) {
            $check['details'][] = [
                ...
                'account_id' => $detail['details_account_id'],
                'account_name_details' => $detail['account_name_details'],
                'details_memo' => $detail['details_memo'],
                'amount' => $detail['amount'],
                'discount_amount' => $detail['discount_amount'],
                'net_amount' => $detail['net_amount'],
                'taxable_amount' => $detail['taxable_amount'],
                'input_vat' => $detail['input_vat']
            ];
        }

        return new WriteCheck($check);
    }
